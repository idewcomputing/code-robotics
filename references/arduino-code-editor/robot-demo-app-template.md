...
* You'll need to add code within the `task1()` function, `task2()` function, and `task3()` function to perform your specific task scenarios. If necessary, you can include a [`while()` loop timer](../robot-behaviors/detecting-other-conditions.md#while-loop-timer) for a task to perform behaviors that must be continuously called \(such as:  avoiding line, etc.\) and stop them after a set duration.
